...
const vltCalc = (result, mltpIdc, vltModel) => {
    if (mltpIdc===KEY_NAME.PER) {
        ...
        if (vltModel===VLT_MODELS.PRICE) {
            ...
            result[KEY_NAME.OPS] = _.round((result[KEY_NAME.OP]*NUM_UNIT.OK)/result[KEY_NAME.SHARE_NUM], 2);
            result[OTHER_KEY_NAME.PRICE] = _.round(result[KEY_NAME.POR]*result[KEY_NAME.OPS], 2);
            result[KEY_NAME.MV] = _.round((result[OTHER_KEY_NAME.PRICE]*result[KEY_NAME.SHARE_NUM])/NUM_UNIT.OK, 2);
        } else if (vltModel===VLT_MODELS.PRFM) {
            result[OTHER_KEY_NAME.PRICE] = _.round((result[KEY_NAME.MV]*NUM_UNIT.OK)/result[KEY_NAME.SHARE_NUM], 2);
            result[KEY_NAME.OPS] =_.round(result[OTHER_KEY_NAME.PRICE]/result[KEY_NAME.POR], 2);
        ...
        if (vltModel===VLT_MODELS.PRICE) {
            result[KEY_NAME.SPS] = _.round((result[KEY_NAME.SALES]*NUM_UNIT.OK)/result[KEY_NAME.SHARE_NUM], 2);
            result[OTHER_KEY_NAME.PRICE] = _.round(result[KEY_NAME.PSR]*result[KEY_NAME.SPS], 2);
            result[KEY_NAME.MV] = _.round((result[OTHER_KEY_NAME.PRICE]*result[KEY_NAME.SHARE_NUM])/NUM_UNIT.OK, 2);
        } else if (vltModel===VLT_MODELS.PRFM) {
            result[OTHER_KEY_NAME.PRICE] = _.round((result[KEY_NAME.MV]*NUM_UNIT.OK)/result[KEY_NAME.SHARE_NUM], 2);
            result[KEY_NAME.SPS] =_.round(result[OTHER_KEY_NAME.PRICE]/result[KEY_NAME.PSR], 2);
    ...
    } else if (mltpIdc===KEY_NAME.EV_EBITDA) {
        if (vltModel===VLT_MODELS.PRICE) {
            result[KEY_NAME.EBITDA] = _.round(result[KEY_NAME.SALES]*(result[KEY_NAME.EPM]/100), 2);
            result[KEY_NAME.EV] = _.round(result[KEY_NAME.EV_EBITDA]*result[KEY_NAME.EBITDA], 2);
            result[KEY_NAME.MV] = _.round(result[KEY_NAME.EV] - result[KEY_NAME.I_B_LBLT], 2);
            result[OTHER_KEY_NAME.PRICE] = _.round((result[KEY_NAME.MV]*NUM_UNIT.OK)/result[KEY_NAME.SHARE_NUM], 2);
        } else if (vltModel===VLT_MODELS.PRFM) {
        } else if (vltModel===VLT_MODELS.MLTP) {
        }     
